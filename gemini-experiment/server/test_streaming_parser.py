...
def test_placeholder_complete():
    ...
    assert items == [
        "<x0>Hello</x0>",
        "<x1/>",
    ], f"Expected complete placeholders, got {items}"
...
def test_split_at_angle_bracket():
    ...
    assert items2 == [
        "Hello <x0>World</x0>"
    ], f"Expected ['Hello <x0>World</x0>'], got {items2}"
...
def test_nested_placeholders():
    ...
    assert items == [
        "<x0><x1>Nested</x1></x0>"
    ], f"Expected nested placeholders, got {items}"
...
def test_yields_complete_strings_with_intact_placeholders():
    ...
    assert (
        "<x0>" in items[0] and "</x0>" in items[0]
    ), "First item missing x0 placeholders"
    assert (
        "<x1>" in items[0] and "</x1>" in items[0]
    ), "First item missing x1 placeholders"
